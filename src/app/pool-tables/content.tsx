...
const showroomProducts = [
  { name: "C.L. Bailey Addison", descriptor: "The table that started a thousand rivalries", image: "/images/pool-tables.jpg" },
  { name: "Brunswick Centurion", descriptor: "Tournament precision for your home", scene: "Brunswick pool table, modern game room, pendant lighting" },
  { name: "Olhausen Americana", descriptor: "Handcrafted heritage, Tennessee soul", scene: "Olhausen table in rustic game room, warm wood tones" },
  { name: "A.E. Schmidt Tradition", descriptor: "Built like they were in 1850", scene: "Classic A.E. Schmidt table, traditional styling" },
  { name: "Connelly Kayenta", descriptor: "Arizona precision meets Kentucky game night", scene: "Connelly table, sleek contemporary game room" },
  { name: "Imperial Lincoln", descriptor: "Game room furniture that plays as good as it looks", scene: "Imperial pool table, family game room setting" },
  { name: "American Heritage Quest", descriptor: "Where furniture design meets serious play", scene: "American Heritage table, elegant home setting" },
]
...
export const PoolTablesContent = () => {
  ...
  return (
    <>
      ...
      <section className="py-[var(--section-py)]" style={{ background: "var(--cat-surface, #F5F0E8)" }}>
        <div className="mx-auto max-w-[var(--content-max)] px-[var(--section-px)]">
          <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-12 lg:gap-20 items-start">
            ...
            >
              <p className="font-bold text-sm mb-6 uppercase tracking-wider" style={{ color: "var(--cat-primary, #1B4332)" }}>
      ...
      {/* ── C.L. BAILEY HERO FEATURE ── deep billiard green */}
      <section style={{ background: "var(--cat-dark, #0D2B1F)" }} className="py-[var(--section-py)]">
        <div className="mx-auto max-w-[var(--content-max)] px-[var(--section-px)]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            >
              <span className="pill mb-4 inline-block" style={{ background: "rgba(201,149,42,0.15)", color: "var(--cat-accent, #C9952A)" }}>Our Lead Brand</span>
              <h2 className="heading text-[length:var(--text-display)] mb-4" style={{ color: "var(--color-text-on-dark)" }}>
                C.L. Bailey
              </h2>
              <p className="text-lg font-semibold mb-6" style={{ color: "var(--cat-accent, #C9952A)" }}>
                {clBailey.tagline}
              </p>
              <p className="text-lg leading-relaxed mb-8 max-w-[55ch]" style={{ color: "rgba(250,246,239,0.65)" }}>
                {clBailey.description}
              </p>
              <div className="border-t pt-6" style={{ borderColor: "rgba(74,124,89,0.3)" }}>
            ...
            >
              {clBaileyLogo ? (
                <div className="relative w-full max-w-[320px] h-[130px]">
                  <Image src={clBaileyLogo} alt="C.L. Bailey logo" fill className="object-contain brightness-0 invert opacity-80" sizes="320px" />
                </div>
              ) : (
                <span className="heading text-[length:var(--text-display)]" style={{ color: "var(--cat-accent, #C9952A)" }}>C.L. Bailey</span>
              )}
            </motion.div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-8">
              {otherBrands.slice(1, 3).map((brand, i) => (
                <motion.div
                  ...
                  style={{ borderColor: "var(--color-border)", background: "var(--cat-surface, var(--color-cream))" }}
                >
                  <div className="flex items-center gap-4 mb-4">
                    {brandLogos[brand.slug] ? (
                      <div className="relative w-[120px] h-[48px] shrink-0">
                        <Image src={brandLogos[brand.slug]} alt={`${brand.name} logo`} fill className="object-contain object-left" sizes="120px" />
                      </div>
                    ) : (
                      <h3 className="heading text-[length:var(--text-h3)]" style={{ color: "var(--color-text)" }}>{brand.name}</h3>
                    )}
                    {brand.established && <span className="pill pill-dark">Est. {brand.established}</span>}
                  </div>
                  {brandLogos[brand.slug] && (
                    <h3 className="heading text-[length:var(--text-h4)] mb-1" style={{ color: "var(--color-text)" }}>{brand.name}</h3>
                  )}
                  <p className="text-sm font-semibold mb-3" style={{ color: "var(--cat-primary, #1B4332)" }}>{brand.tagline}</p>
      ...
      <section className="py-[var(--section-py)]">
        <div className="mx-auto max-w-[var(--content-max)] px-[var(--section-px)]">
          <SectionHeading label="In Our Showroom" headline="What's on the Floor" description="Come in, walk around them, run your hand across the felt. This is how you buy a pool table." />
          <div className="space-y-6">
            ...
            >
              <div className="relative aspect-[21/9] overflow-hidden">
                <Image src={showroomProducts[0].image!} alt={showroomProducts[0].name} fill className="object-cover transition-transform duration-[1.5s] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-105" sizes="100vw" />
                <div className="absolute inset-0" style={{ background: "linear-gradient(to top, var(--cat-dark, #0D2B1F) 0%, rgba(13,43,31,0.2) 50%, transparent 100%)" }} />
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-12">
                <span className="pill mb-3 inline-block" style={{ background: "rgba(201,149,42,0.15)", color: "var(--cat-accent, #C9952A)" }}>Most Popular</span>
                <h3 className="heading text-[length:var(--text-h2)] mb-2" style={{ color: "var(--color-text-on-dark)" }}>{showroomProducts[0].name}</h3>
                <p className="text-base mb-6" style={{ color: "rgba(250,246,239,0.65)" }}>{showroomProducts[0].descriptor}</p>
              ...
              {showroomProducts.slice(1, 5).map((product, i) => (
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute inset-0" style={{ background: "linear-gradient(to top, var(--cat-dark, #0D2B1F) 0%, transparent 60%)" }} />
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    <h4 className="heading text-[length:var(--text-h4)] mb-1" style={{ color: "var(--color-text-on-dark)" }}>{product.name}</h4>
                    <p className="text-sm mb-4" style={{ color: "rgba(250,246,239,0.6)" }}>{product.descriptor}</p>
                    <span className="text-sm font-bold" style={{ color: "var(--cat-accent, #C9952A)" }}>Call for Pricing &rarr;</span>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {showroomProducts.slice(5).map((product, i) => (
                ...
                >
                  ...
                  <div className="p-6">
                    <h4 className="heading text-[length:var(--text-h4)] mb-1" style={{ color: "var(--color-text)" }}>{product.name}</h4>
                    <p className="text-sm mb-4" style={{ color: "var(--color-text-secondary)" }}>{product.descriptor}</p>
                    <span className="text-sm font-bold" style={{ color: "var(--cat-secondary, #8B5E3C)" }}>Call for Pricing &rarr;</span>
                  </div>
                </motion.div>
              ))}
            </div>
